#!/bin/bash
# ScholarClaw - Recommend Papers Script
...
Options:
    -l, --limit NUMBER        Number of papers to return (default: 12, max: 50)
...
# Build URL
URL="${SERVER_URL}/api/recommend/papers?limit=${LIMIT}"

# Execute request
echo "Fetching trending papers..." >&2
...
if [[ "$HTTP_CODE" -ne 200 ]]; then
    handle_http_error "$HTTP_CODE" "$BODY" "Fetch papers"
    exit 1
fi
